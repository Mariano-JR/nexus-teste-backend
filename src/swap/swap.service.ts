...
export class SwapService {
    ...
    async getQuote(data: SwapDto) {
        const price = await this.coingeckoService.getPrice(data.tokenIn, data.tokenOut);
    ...
    private async swapOut(tx: PrismaClient, userId: string, data: SwapDto) {
        const coinBalance = await this.walletService.getCoinBalance(userId, data.tokenOut);
    ...
    private async swapFee(tx: PrismaClient, userId: string, data: SwapDto, calculatedValues: SwapCalculation) {
        const coinBalance = await this.walletService.getCoinBalance(userId, data.tokenIn);
    ...
    private async swapIn(tx: PrismaClient, userId: string, data: SwapDto, calculatedValues: SwapCalculation) {
        const coinBalance = await this.walletService.getCoinBalance(userId, data.tokenIn);
